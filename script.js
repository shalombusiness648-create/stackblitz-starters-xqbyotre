...
const clientSupabase = window.supabase.createClient(
  window.SBC_SUPABASE_URL,
  window.SBC_SUPABASE_KEY
);
...
async function chargerProduits() {
  try {
    ...
    document.getElementById(
      'pile-container'
    ).innerHTML = `<p class="text-rose-500 font-medium text-center p-4">❌ Impossible de charger les produits. Vérifiez la configuration.</p>`;
  }
}

// 2. AFFICHAGE DES CARTES (PILE TINDER + CARROUSEL AUTOMATIQUE)
function afficherPile() {
  const container = document.getElementById('pile-container');
  container.innerHTML = '';

  if (indexActuel >= produitsListe.length) {
    container.innerHTML = `
            <div class="text-center p-6 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                ...
                <p class="text-slate-500 font-medium">Vous avez vu tous nos produits !</p>
                <button onclick="recommencerSwipe()" class="mt-4 px-4 py-2 bg-emerald-500 text-white font-semibold rounded-xl shadow-md hover:bg-emerald-600 active:scale-95 transition-all text-sm">Revoir le catalogue</button>
            </div>`;
    return;
  }

  produitsListe.forEach((produit, index) => {
    if (index < indexActuel) return;

    const carte = document.createElement('div');
    carte.id = `carte-${index}`;
    carte.className =
      'absolute w-full h-full bg-white rounded-2xl shadow-md border border-slate-200 p-5 flex flex-col justify-between transition-all duration-300 origin-bottom';
    carte.style.zIndex = produitsListe.length - index;

    if (index > indexActuel) {
      carte.style.transform = `scale(${
        1 - (index - indexActuel) * 0.04
      }) translateY(${(index - indexActuel) * 12}px)`;
      carte.style.opacity = index - indexActuel > 2 ? 0 : 0.8;
    }

    // Gestion du Carrousel d'images (Images de test si vide)
    const sampleImages = [
    ...
    listeImages.forEach((imgUrl, imgIndex) => {
      imagesHtml += `<img src="${imgUrl}" id="img-${index}-${imgIndex}" class="carrousel-img absolute inset-0 w-full h-full object-contain transition-opacity duration-500 ${
        imgIndex === 0 ? 'opacity-100' : 'opacity-0'
      }">`;
      indicateursHtml += `<div id="ind-${index}-${imgIndex}" class="h-1 flex-1 rounded-full transition-all ${
        imgIndex === 0 ? 'bg-emerald-500' : 'bg-slate-200'
      }"></div>`;
    });
    ...
    carte.innerHTML = `
            ...
            <div id="face-avant-${index}" class="flex-1 flex flex-col justify-between h-full">
                ...
                <div class="mt-4">
                    ...
                    <h2 class="text-xl font-bold text-slate-800 mt-1 leading-tight">${
                      produit.nom
                    }</h2>
                    ...
                    <div class="flex justify-between items-baseline mt-4 pt-3 border-t border-slate-100">
                        ...
                        <span class="text-2xl font-black text-emerald-600">${
                          produit.prix_rmb
                        } <span class="text-sm font-bold">RMB</span></span>
                    </div>
                </div>
            </div>
            ...
            <div id="face-arriere-${index}" class="hidden flex-1 flex flex-col justify-between h-full overflow-y-auto pt-4">
                <div>
                    ...
                    <div class="text-sm text-slate-600 space-y-2 whitespace-pre-line bg-slate-50 p-3 rounded-xl">${
                      produit.parametres || 'Aucun paramètre spécifié.'
                    }</div>
                    ...
                    <div class="grid grid-cols-2 gap-3 text-xs">
                        <div class="bg-slate-50 p-2 rounded-lg text-center"><p class="text-slate-400 font-medium">Colisage</p><p class="text-slate-700 font-bold text-sm mt-0.5">${
                          produit.qty_ctn || '-'
                        }</p></div>
                        <div class="bg-slate-50 p-2 rounded-lg text-center"><p class="text-slate-400 font-medium">Volume (CBM)</p><p class="text-slate-700 font-bold text-sm mt-0.5">${
                          produit.cbm || '-'
                        }</p></div>
                    </div>
                </div>
    ...
    if (index === indexActuel && listeImages.length > 1) {
      ...
      window.carrouselTimer = setInterval(() => {
        ...
        const nextImgEl = document.getElementById(
          `img-${index}-${imgActuelle}`
        );
        const nextIndEl = document.getElementById(
          `ind-${index}-${imgActuelle}`
        );
